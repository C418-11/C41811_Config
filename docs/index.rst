C41811.Config 的文档
==================================

.. image:: https://img.shields.io/pypi/pyversions/c41811.config.svg
   :target: https://pypi.python.org/pypi/C41811.Config/
   :alt: 支持的python版本

.. image:: https://img.shields.io/pypi/l/C41811.Config?color=blue
   :target: https://github.com/C418-11/C41811_Config/blob/main/LICENSE
   :alt: 开源许可证

.. list-table::
   :widths: auto
   :align: center

   * - 文档
     -
       .. image:: https://readthedocs.org/projects/c41811config/badge/?version=latest
          :target: https://app.readthedocs.org/projects/c41811config/
          :alt: 文档状态

       .. image:: https://img.shields.io/badge/%E6%96%B0%E6%89%8B-%E6%95%99%E7%A8%8B-green?logo=googledocs&logoColor=white
          :target: ./Tutorial/get-start.rst
          :alt: 新手教程

       .. image:: https://img.shields.io/badge/%E5%B8%B8%E8%A7%81-%E9%97%AE%E9%A2%98-green?logo=googledocs&logoColor=white
          :target: ./Tutorial/faq.html
          :alt: 常见问题
   * - PyPI
     -
       .. image:: https://img.shields.io/pypi/v/C41811.Config
          :target: https://pypi.python.org/pypi/C41811.Config/
          :alt: PyPI包版本

       .. image:: https://img.shields.io/pypi/wheel/C41811.Config
          :target: https://pypi.python.org/pypi/C41811.Config/
          :alt: PyPI - Wheel

       .. image:: https://img.shields.io/pypi/dm/c41811.config.svg
          :target: https://pypi.python.org/pypi/C41811.Config/
          :alt: PyPI每月下载量
   * - 仓库
     -
       .. image:: https://img.shields.io/badge/Github-C41811.Config-green?logo=github
          :target: https://github.com/C418-11/C41811_Config/
          :alt: Github

       .. image:: https://img.shields.io/github/actions/workflow/status/C418-11/C41811_Config/python-publish.yml?logo=github&label=Pubilsh
          :target: https://github.com/C418-11/C41811_Config/actions/workflows/python-publish.yml
          :alt: 发布工作流状态
   * - 代码质量-主分支
     -
       .. image:: https://img.shields.io/github/actions/workflow/status/C418-11/C41811_Config/python-ci.yml?branch=main&logo=github&label=CI
          :target: https://github.com/C418-11/C41811_Config/actions/workflows/python-ci.yml?query=branch%3Amain
          :alt: CI状态

       .. image:: https://codecov.io/gh/C418-11/C41811_Config/branch/main/graph/badge.svg
          :target: https://codecov.io/gh/C418-11/C41811_Config/tree/main
          :alt: CodeCov覆盖率
   * - 代码质量-开发分支
     -
       .. image:: https://img.shields.io/github/actions/workflow/status/C418-11/C41811_Config/python-ci.yml?branch=develop&logo=github&label=CI
          :target: https://github.com/C418-11/C41811_Config/actions/workflows/python-ci.yml?query=branch%3Adevelop
          :alt: CI状态

       .. image:: https://codecov.io/gh/C418-11/C41811_Config/branch/develop/graph/badge.svg
          :target: https://codecov.io/gh/C418-11/C41811_Config/tree/develop
          :alt: CodeCov覆盖率

.. toctree::
   :maxdepth: 2

   /Tutorial/index
   project-specification
   reports

索引和表
========

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`
